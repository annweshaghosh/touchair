...
namespace BoilerPlate_Project.Tests
{
    public abstract class BoilerPlate_ProjectTestBase : AbpIntegratedTestBase<BoilerPlate_ProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BoilerPlate_ProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<BoilerPlate_ProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BoilerPlate_ProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BoilerPlate_ProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BoilerPlate_ProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BoilerPlate_ProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerPlate_ProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BoilerPlate_ProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerPlate_ProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BoilerPlate_ProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BoilerPlate_ProjectDbContext, Task<T>> func)
        {
            T result;
